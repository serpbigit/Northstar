...
**2. Create `docs/bundledFiles.ts`:**

This command will create a single file containing all of your project's TypeScript code, which is useful for sharing with other team members or AI agents.

```bash
cat > docs/bundledFiles.ts << 'EOT'
    ...
    try {
        ...
        for (let i = 1; i < data.length; i++) {
            if (data[i][actionIdCol] === actionId) {
                ...
                if (status !== 'PENDING') {
                    return { ok: false, error: `Action  status is ''. Cannot execute.` };
                }
...
function onMessage(event: GoogleAppsScript.Events.ChatEvent) {
  try {
    ...
    if (!route.ok) {
      const userErr = route.reason === 'no-match' ? `🤖 Echo: ` : `Router error: ${route.reason}`;
...
function cmd_HandleSheetData_(params: SpecialistParams): SpecialistResult {
  try {
    ...
    if (addMatch) {
      ...
      return { ok: true, message: `✅ Added to : ` };
    }
...
function calendar_buildEventLink_(event: GoogleAppsScript.Calendar.CalendarEvent): string {
  try {
    ...
    const b64_eid = Utilities.base64Encode(` `);
...
function calendar_read_(cmd: CalendarReadCommand): SpecialistResult {
  try {
    ...
    if (!events.length) {
      ...
      const rangeStr = (startStr === endStr) ? startStr : ` to `;
      const message = lang === 'he' ? `🗓️ לא נמצאו אירועים עבור .` : `🗓️ No events found for .`;
...
function calendar_create_(cmd: CalendarCreateCommand): SpecialistResult {
  try {
    ...
    const message = lang === 'he'
      ? `✅ אירוע נוצר בהצלחה: *${event.getTitle()}* ב- בשעה .`
      : `✅ Event created successfully: *${event.getTitle()}* on  at .`;
    return { ok: true, message };
  } catch (e) {
...
function cmd_HandleTasks_(params: SpecialistParams): SpecialistResult {
  const text = params.text || '';
  return { ok: true, message: `🤖 **Tasks Stub:** I will soon handle: ""` };
}
...
function gmail_read_(cmd: GmailReadCommand): SpecialistResult {
  try {
    ...
    if (!threads.length) {
      const message = lang === 'he' ? `📭 לא נמצאו אימיילים עבור השאילתה: ""` : `📭 No emails found for query: ""`;
      return { ok: true, message };
    }
...
function gmail_send_(cmd: { to: string, subject: string, body: string, reply_lang?: Language }): { ok: boolean, message?: string, error?: string } {
  try {
    ...
    const successMsg = lang === 'he' ? `✅ נשלח בהצלחה אל: ` : `✅ Successfully sent to: `;
...
-> {"action": "draft", "to": "bob", "subject": null, "body": null, "reply_lang": "en"}`;
    ...
    switch (cmd.action) {
      ...
      case 'draft':
        ...
        const approvalUrl = `${ScriptApp.getService().getUrl()}?action=gmail_send_confirm&id=`;
...
function buildCodebaseHtml_(): string {
  try {
    ...
    data[0].forEach(header => { html += `<th></th>`; });
    html += '</tr></thead><tbody>';
    for (let i = 1; i < data.length; i++) {
      html += '<tr>';
...
function cmd_HandlePolicyResponse_({ actionId, status, source }: { actionId: string; status: string; source: string }): void {
  ...
  try {
    ...
    if (!record) {
      Logger.log(`WARN: Action ID [] not found. The link may have expired or been used already.`);
      util_SendChatMessageText_(source, 'The approval link has expired or was already used.');
      return;
    }
    ...
    // Prevent double execution.
    if (record.Status !== 'PENDING') {
      Logger.log(`WARN: Action ID [] has status [${record.Status}] and is not PENDING. No action taken.`);
      util_SendChatMessageText_(source, `This action was already processed with status: ${record.Status}.`);
      return;
    }
    ...
    // Step 3: Execute or reject the action.
    if (status === 'APPROVED') {
      Logger.log(`Action [] APPROVED. Executing handler: ${record.HandlerKey}`);
      const payload = JSON.parse(record.ActionPayload as string);
      ...
      if (typeof handler === 'function') {
        ...
        Logger.log(`Successfully executed handler [${record.HandlerKey}] for action [].`);
      ...
      // status === 'REJECTED'
      Logger.log(`Action [] REJECTED by user.`);
...
function util_getPendingAction_(actionId: string, spreadsheetId: string): { record: Record<string, any> | null; rowIndex: number; headers: string[] } {
  ...
  Logger.log(`INFO: Action ID [] not found in '${CFG_PENDING_.SHEET}' sheet.`);
  return { record: null, rowIndex: -1, headers: [] };
}
...
function util_SendChatMessageText_(spaceName: string, text: string): void {
  ...
  Logger.log(`Simulating message to Chat space []: ""`);
