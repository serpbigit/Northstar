...
# --- 2. COMPILATION ---
# Use the TypeScript compiler (tsc) with our configuration
echo "--- Compiling TypeScript files (TS -> JS) ---"
# We must temporarily output to the 'dist' directory. The tsconfig.json handles which files to compile.
tsc --project "$TSCONFIG_FILE" --outDir "$OUTPUT_DIR"
if [ $? -ne 0 ]; then
...
# IMPORTANT: The 'sort' command is crucial here to ensure files are processed 
# in order (01, 02, 03, etc.) to respect function dependencies.
find "$OUTPUT_DIR" -name "*.js" | sort | while read FILE; do
    echo "" >> "$OUTPUT_FILE"
    # Use the base filename for the header, stripping the path and extension
    BASE_FILENAME=$(basename "$FILE" .js)
    echo "// FILE: $BASE_FILENAME.ts" >> "$OUTPUT_FILE"
    echo "/* --- BLOCK START --- */" >> "$OUTPUT_FILE"
    
    # Concatenate the content, then remove the '"use strict";' line often added by tsc
